...
async function api(req, res, url) {
  ...
  if (resource === 'health' && method === 'GET') {
    return json(res, 200, { ok: true, app: 'zen-human-server', auth: AUTH_ON, ...counts() });
  }
  ...
  // Stato completo: GET per il boot (qualsiasi autenticato), PUT per sostituzione totale.
  if (resource === 'data') {
...
createServer(async (req, res) => {
  ...
  console.log(`\n  Zen-Human — server dati (v${updater.currentVersion(__dirname)})`);
