// ============ (De)serializzazione DB ⇄ modello JSON dell'app (Zen-Human) ============
...
function stripChildren(c, o) { const clone = { ...o }; for (const k of childKeys(c)) delete clone[k]; return clone; }

function insertMain(c, o) {
  const names = c.cols.map((x) => x.n);
  const cols = ['id', ...names, 'doc'];
  const vals = [o.id ?? null, ...c.cols.map((cs) => colValue(cs, o)), JSON.stringify(stripChildren(c, o))];
...
export function importData(json, { force = false } = {}) {
  // Guardia sull'input GREZZO (prima di migrate): per Zen-Human il marcatore è `companies`.
  if (!json || typeof json !== 'object' || !Array.isArray(json.companies))
